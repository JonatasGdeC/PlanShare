...
internal sealed class UserRepository : IUserWriteOnlyRepository, IUserReadOnlyRepository, IUserUpdateOnlyRepository
{
    ...
    public async Task Add(User user) => await _dbContext.Users.AddAsync(entity: user);

    public async Task<bool> ExistActiveUserWithEmail(string email) => await _dbContext.Users.AnyAsync(predicate: user => user.Email.Equals(email) && user.Active);
    ...
    public void Update(User user) => _dbContext.Users.Update(entity: user);
}